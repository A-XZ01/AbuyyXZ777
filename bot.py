...
@client.tree.command(
    name="complete",
    description="Catat transaksi selesai: pilih item dan harga otomatis terisi."
)
...
async def complete(interaction: discord.Interaction, item: str, user: Optional[discord.Member] = None):
    ...
    db.add_transaction(interaction.guild.id, target.id, amount, category="manual", notes=f"Via /complete by {interaction.user.name}", recorded_by=interaction.user.id)
    
    # Cek achievement baru
    new_achievements = db.check_and_unlock_achievement(interaction.guild.id, target.id)
    
    # Log action
    db.log_action(interaction.guild.id, interaction.user.id, "complete_deal", f"Target: {target.id}, Amount: {amount}")
    ...
    embed.add_field(
        ...
        value=f"**{updated.get('deals_completed', 0)}** deals", 
        inline=True
    )
    embed.add_field(
        name="💰 Total Belanja", 
        value=f"**{format_idr(updated.get('total_idr_value', 0))}**", 
        inline=True
    )
    ...
    if new_achievements:
        ...
        achievement_text = "\n".join([f"• {achievement_names.get(a, a)}" for a in new_achievements])
