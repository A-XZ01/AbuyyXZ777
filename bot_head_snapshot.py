...
class UsernameModal(discord.ui.Modal, title="=ƒÄ½ Create New Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if len(username) < 3:
            await interaction.followup.send(
                "G¥î Username game minimal 3 karakter!",
                ephemeral=True
            )
            return
        ...
        if existing_ticket:
            ...
            if channel:
                ...
                await interaction.followup.send(
                    f"G¥î Anda sudah punya ticket aktif: {channel.mention}\n"
                ...
                print(f"GÜán+Å Auto-closed orphaned ticket #{existing_ticket['ticket_number']} (channel deleted)")
        ...
        if not category:
            try:
                ...
                await interaction.followup.send(
                    "G¥î Bot tidak punya permission untuk membuat kategori.\n"
        ...
        try:
            ...
            if not ticket_id:
                ...
                await interaction.followup.send(
                    "G¥î Gagal membuat ticket di database. Coba lagi.",
                    ephemeral=True
                )
                return
            ...
            welcome_embed = discord.Embed(
                title=f"=ƒÄ½ Ticket #{ticket_number:04d}",
            ...
            items_text = []
            for item in items:
                items_text.append(f"**{item['name']}:** {item['robux']} R$ GÇó Rp{item['price_idr']:,}")
            
            welcome_embed.add_field(
                name=f"=ƒÆÄ Item & Harga (Rate: Rp{rate}/Robux)",
                value="\n".join(items_text),
                inline=False
            )
            
            welcome_embed.set_footer(text=f"Ticket #{ticket_number:04d} GÇó 4-Layer Fraud Detection Active")
            ...
            class ItemSelectView(discord.ui.View):
                ...
                @discord.ui.select(
                    placeholder="=ƒ¢Æ Pilih item yang ingin dibeli...",
                    min_values=1,
                    max_values=1,
                ...
                async def select_callback(self, interaction: discord.Interaction, select: discord.ui.Select):
                    ...
                    if not item_data:
                        await interaction.response.send_message("G¥î Item tidak ditemukan.", ephemeral=True)
                        return
            ...
            for item in items[:25]:  # Discord max 25 options
                options.append(
                    SelectOption(
                        label=item['name'],
                        value=item['code'],
                        description=f"{item['robux']} R$ GÇó Rp{item['price_idr']:,}",
                        emoji="=ƒÄ«"
                    )
                )
            ...
            add_prompt_embed = discord.Embed(
                title="=ƒ¢ìn+Å Pilih Item",
                description=(
                    f"{interaction.user.mention}\n\n"
                    "G¼çn+Å **Pilih item dari menu di bawah ini:**\n\n"
                    "=ƒö¦ Klik dropdown menu\n"
                    "=ƒö¦ Pilih item yang diinginkan\n"
                    "=ƒö¦ Masukkan jumlah (quantity)\n"
                    "=ƒö¦ Ulangi untuk item lain jika perlu"
                ),
            ...
            notify_message = await interaction.followup.send(
                f"G£à Ticket berhasil dibuat!\n\n"
                f"=ƒÄ½ **Ticket:** {channel.mention}\n"
                f"=ƒô¥ **Username:** `{username}`\n\n"
            ...
            await interaction.followup.send(
                "G¥î Bot tidak punya permission untuk membuat channel.\n"
                "Enable `Manage Channels` permission untuk bot role.",
                ephemeral=True
            )
        except Exception as e:
            await interaction.followup.send(
                f"G¥î Error membuat ticket: {e}",
                ephemeral=True
            )
...
class QuantityModal(discord.ui.Modal):
    ...
    def __init__(self, item_data: dict, ticket_id: int):
        super().__init__(title=f"=ƒôè Jumlah: {item_data['name']}")
    ...
    async def on_submit(self, modal_interaction: discord.Interaction):
        ...
        try:
            ...
            for item in ticket_items:
                items_text.append(
                    f"GÇó **{item['item_name']}** = Rp{item['amount']:,}"
                )
            
            embed = discord.Embed(
                title="G£à Item berhasil ditambahkan!",
                description=f"**{self.item_data['name']}** x{qty} = Rp{total:,}",
                color=discord.Color.green()
            )
            
            embed.add_field(
                name="=ƒôï Keranjang Belanja",
                value="\n".join(items_text),
                inline=False
            )
            ...
            await modal_interaction.followup.send(
                "G¥î Input tidak valid. Masukkan angka 1-5.",
                ephemeral=True
            )
        except Exception as e:
            print(f"G¥î Error in QuantityModal.on_submit: {e}")
            import traceback
            traceback.print_exc()
            await modal_interaction.followup.send(
                f"G¥î Terjadi error: {e}",
                ephemeral=True
            )
...
class MiddlemanModal(discord.ui.Modal, title="=ƒñ¥ Create Middleman Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if deal_price < 1000:
                await interaction.followup.send("G¥î Harga minimal Rp1.000!", ephemeral=True)
                return
            if deal_price > 100000000:
                await interaction.followup.send("G¥î Harga maksimal Rp100.000.000!", ephemeral=True)
                return
        except ValueError:
            await interaction.followup.send("G¥î Harga harus berupa angka yang valid!", ephemeral=True)
            return
        ...
        if self.fee_payer == 'split' and deal_price < 5000000:
            await interaction.followup.send(
                "G¥î **Split Fee hanya tersedia untuk transaksi di atas Rp5.000.000!**\n"
        ...
        try:
            ...
            await channel.edit(
                ...
                topic=f"Middleman Ticket #{ticket_number:04d} - {interaction.user.name} Gåö {seller_username}"
            )
            
            # Welcome embed untuk middleman
            welcome_embed = discord.Embed(
                title=f"=ƒñ¥ Middleman Ticket #{ticket_number:04d}",
            ...
            welcome_embed.add_field(
                name="=ƒÆ¦ Rincian Pembayaran",
                value=(
                    f"Harga Deal: **Rp{deal_price:,}**\n"
                    f"Fee Middleman: Rp{mm_fee:,} {fee_info}\n"
                    f"GöüGöüGöüGöüGöüGöüGöüGöüGöüGöüGöüGöüGöüGöüGöü\n"
            ...
            # WARNING - Simplified
            welcome_embed.add_field(
                name="GÜán+Å Bukti Transfer",
                value=(
                    f"G£à Screenshot FULL & ASLI (semua detail terlihat)\n"
                    f"G¥î DILARANG: Crop/blur/edit/fake screenshot\n\n"
                    f"GÜí **Fraud Detection Active** - Bukti palsu ditolak otomatis"
                ),
                inline=False
            )
            
            welcome_embed.set_footer(text=f"Middleman Ticket #{ticket_number:04d} GÇó Waiting Buyer Payment | {datetime.now().strftime('%d %b %Y, %H:%M')}")
            ...
            await interaction.followup.send(
                f"G£à Middleman ticket berhasil dibuat!\n\n"
                f"=ƒñ¥ **Ticket:** {channel.mention}\n"
                f"=ƒÆ¦ **Buyer Transfer:** Rp{total_payment:,}\n"
                f"=ƒÆ¦ **Seller Terima:** Rp{seller_receives:,}\n"
                f"=ƒôè **{fee_note}**\n\n"
...
class FeePayerSelectView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="=ƒôè Pilih siapa yang bayar fee middleman...",
        options=[
...
class CreateMiddlemanButton(discord.ui.View):
    ...
    async def create_mm_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="=ƒñ¥ Middleman Ticket - Fee Payment",
            description=(
                "Selamat datang di layanan Middleman!\n\n"
                "Silakan pilih **siapa yang akan membayar fee middleman**:\n\n"
                "=ƒö¦ **Buyer Pays** - Buyer bayar harga deal + fee penuh\n"
                "=ƒƒó **Seller Pays** - Seller terima harga deal - fee\n"
                "=ƒƒí **Split 50:50** - Fee dibagi rata (hanya untuk >Rp5.000.000)\n\n"
                "**Fee Structure:**\n"
                "GÇó <50K: Gratis\n"
                "GÇó 50K-500K: Rp2.000\n"
                "GÇó 500K-1Juta: Rp5.000\n"
                "GÇó 1Juta-5Juta: Rp7.000\n"
                "GÇó 5Juta-10Juta: Rp10.000\n"
                "GÇó 10Juta+: Rp15.000"
            ),
            color=0xFF9900
        )
...
def calculate_mm_fee(deal_price: int) -> int:
    ...
    if deal_price < 50000:
        ...
        return 7000  # UPDATED: dari 7500 GåÆ 7000
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(image_url) as resp:
                ...
                print(f"=ƒöì HYBRID LEGITIMACY CHECK...")
                ...
                try:
                    ...
                    print(f"=ƒô¥ OCR Text extracted: {len(text)} chars")
                    ...
                    for keyword, points in banking_keywords.items():
                        if keyword in text:
                            ...
                            print(f"   G£à Found keyword: {keyword} (+{points} points)")
                    ...
                    if account_numbers:
                        ...
                        print(f"   G£à Found {len(account_numbers)} account numbers (+20 points)")
                    
                    # Detect currency (Rp)
                    if 'RP' in text or 'IDR' in text:
                        legitimate_indicators.append("OCR: Indonesian currency")
                        confidence_scores.append(15)
                        print(f"   G£à Currency detected (+15 points)")
                        
                except Exception as ocr_error:
                    print(f"   GÜán+Å OCR unavailable (Tesseract not installed)")
                ...
                for r_min, r_max, g_min, g_max, b_min, b_max, name, points in banking_colors:
                    if r_min <= avg_r <= r_max and g_min <= avg_g <= g_max and b_min <= avg_b <= b_max:
                        ...
                        confidence_scores.append(points)
                        print(f"   =ƒÄ¿ Detected {name} (+{points} points)")
                ...
                for width, (h_min, h_max, device_name, points) in device_resolutions.items():
                    if w == width and h_min <= h <= h_max:
                        ...
                        confidence_scores.append(points)
                        print(f"   =ƒô¦ Detected {device_name} ({w}x{h}) (+{points} points)")
                        break
                ...
                if 5 <= text_density <= 15:
                    ...
                    confidence_scores.append(20)
                    print(f"   =ƒôä Receipt text density: {text_density:.2f} (+20 points)")
                ...
                if 30 <= center_std <= 70:
                    ...
                    confidence_scores.append(15)
                    print(f"   =ƒöÆ Watermark detected (+15 points)")
                ...
                if status_std < 20:
                    ...
                    print(f"   =ƒô¦ Mobile status bar (+10 points)")
                ...
                if is_legitimate:
                    print(f"G£à LEGITIMATE SCREENSHOT! Confidence: {total_confidence:.1f}%")
                    print(f"   Indicators: {len(legitimate_indicators)}")
                else:
                    print(f"GÜán+Å Screenshot unclear (confidence: {total_confidence:.1f}%)")
        ...
        print(f"GÜán+Å Error checking legitimate screenshot: {e}")
        import traceback
        traceback.print_exc()
        return {'is_legitimate': False, 'source': 'unknown', 'confidence': 0}
...
async def detect_image_manipulation(image_url: str) -> dict:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(image_url) as resp:
                if resp.status != 200:
                    print(f"GÜán+Å Failed to download image for manipulation check: HTTP {resp.status}")
                    return {'manipulation_detected': False, 'warnings': [], 'confidence': 0}
                
                image_data = await resp.read()
                image = Image.open(io.BytesIO(image_data))
                
                print(f"=ƒô+ Image loaded: {image.size} ({image.mode})")
                ...
                print(f"=ƒöì Analyzing {w}x{h} image ({total_pixels:,} pixels)...")
                ...
                print(f"   =ƒÄ¿ Scribble detection: {scribble_pixels:,} pixels ({scribble_ratio*100:.2f}%)")
                ...
                if scribble_ratio > 0.05:
                    warnings.append(f"=ƒÄ¿ **Terdeteksi coretan/gambar warna** ({scribble_ratio*100:.1f}% area)")
                ...
                print(f"   =ƒƒª Mosaic detection: {uniform_blocks}/{total_blocks} uniform blocks ({mosaic_ratio*100:.2f}%)")
                print(f"   G£¿ Detail blocks: {high_variance_blocks}/{total_blocks} ({detail_ratio*100:.2f}%)")
                
                # SMART DETECTION:
                # - If >95% uniform AND <5% detail GåÆ Likely intentional mosaic/blur
                # - If 70-95% uniform AND >10% detail GåÆ Natural low-res screenshot (OK)
                if mosaic_ratio > 0.95 and detail_ratio < 0.05:
                    warnings.append(f"=ƒƒª **Mosaik/blur terdeteksi** ({mosaic_ratio*100:.1f}% area ter-blur)")
                ...
                print(f"   =ƒôÉ Edge sharpness: {avg_gradient:.2f}")
                
                # LOWERED threshold: Only flag if VERY blurry (<2.0)
                # Screenshot HP typically 3-6, heavily blurred <2
                if avg_gradient < 2.0:
                    warnings.append(f"=ƒî½n+Å **Gambar sangat blur** (sharpness: {avg_gradient:.1f})")
                ...
                print(f"   GÜán+Å Warnings: {len(warnings)}, Confidence: {total_confidence:.1f}%")
                
                if manipulation_detected:
                    print(f"=ƒÜ¿ IMAGE MANIPULATION DETECTED!")
        ...
        print(f"GÜán+Å Error detecting manipulation: {e}")
        import traceback
        traceback.print_exc()
        return {'manipulation_detected': False, 'warnings': [], 'confidence': 0}
...
async def get_image_hash(image_url: str) -> dict:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(image_url) as resp:
                if resp.status != 200:
                    print(f"GÜán+Å Failed to download image: HTTP {resp.status}")
                    return None
                ...
                print(f"G£à Generated image hashes:")
                print(f"   GÇó dhash: {dhash_result[:16]}... (gradient-based)")
                print(f"   GÇó phash: {phash_result[:16]}... (DCT frequency)")
                print(f"   GÇó ahash: {ahash_result[:16]}... (brightness)")
        ...
        print(f"GÜán+Å Error hashing image: {e}")
        import traceback
        traceback.print_exc()
        return None
...
async def check_similar_images(guild_id: int, new_hashes: dict, current_ticket_id: int = None) -> dict:
    ...
    try:
        ...
        print(f"=ƒöì Multi-algorithm similarity check: {len(all_tickets)} tickets in database")
        ...
        for ticket in all_tickets:
            ...
            print(f"=ƒôè Ticket #{ticket['ticket_number']:04d}:")
            print(f"   GÇó dhash: {dhash_dist} bits (threshold 25)")
            print(f"   GÇó phash: {phash_dist} bits (threshold 30)")
            print(f"   GÇó ahash: {ahash_dist} bits (threshold 15)")
            ...
            if matched:
                ...
                if is_same_ticket:
                    print(f"=ƒÜ¿ SAME TICKET DUPLICATE! User trying to replace proof with SAME image!")
                ...
                print(f"=ƒÜ¿ DUPLICATE DETECTED via {match_algorithm}!")
                print(f"   Similarity: {similarity_pct:.1f}% (distance: {best_distance} bits)")
        ...
        print(f"G£à No similar images found (checked {len(all_tickets)} tickets)")
        return None
        
    except Exception as e:
        print(f"GÜán+Å Error checking similar images: {e}")
        import traceback
        traceback.print_exc()
        return None
...
async def extract_amount_from_image(image_url: str) -> Optional[int]:
    ...
    try:
        ...
        print(f"GÜán+Å Error OCR amount detection: {e}")
        return None
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(image_url) as resp:
                ...
                if account_number and amount:
                    ...
                    print(f"G£à Transfer signature: {signature}")
                    return signature
                
                return None
                
    except Exception as e:
        print(f"GÜán+Å Error extracting transfer signature: {e}")
        return None
...
class MyClient(discord.Client):
    ...
    async def on_ready(self):
        print(f'G£à Bot berhasil Login sebagai {self.user} (ID: {self.user.id})')
        print(f"=ƒôí Bot aktif di {len(self.guilds)} server")
        for guild in self.guilds:
            print(f"   - {guild.name} (ID: {guild.id})")
            # Auto-leave jika bukan server yang diizinkan
            if guild.id not in ALLOWED_GUILDS:
                print(f"GÜán+Å Server {guild.name} tidak ada di whitelist, keluar...")
                await guild.leave()
                print(f"G£à Bot keluar dari server {guild.name}")
        print("GÅ¦ Mencoba sinkronisasi Slash Commands...")
        try:
            # Sync to specific guilds first (faster)
            for guild in self.guilds:
                if guild.id in ALLOWED_GUILDS:
                    synced_guild = await self.tree.sync(guild=guild)
                    print(f"=ƒÄë {len(synced_guild)} Commands synced to {guild.name}")
            
            # Then global sync as backup
            synced = await self.tree.sync()
            print(f"=ƒÄë {len(synced)} Slash Commands synced globally!")
            print("=ƒÆí Commands sekarang tersedia di semua server!")
        except Exception as e:
            print(f"G¥î Gagal sinkronisasi commands: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def auto_backup_task(self):
        ...
        try:
            print("=ƒöä Menjalankan auto-backup...")
            ...
            for guild in self.guilds:
                ...
                print(f"G£à Auto-backup selesai untuk {guild.name}: {backup_file}")
            ...
            for old_file in files[30:]:
                try:
                    os.remove(old_file)
                    print(f"=ƒùæn+Å Hapus backup lama: {old_file}")
                except:
                    pass
        except Exception as e:
            print(f"G¥î Error saat auto-backup: {e}")
        ...
        try:
            for guild in self.guilds:
                ...
                try:
                    ...
                    print(f"G£à Leaderboard auto-updated untuk {guild.name}")
                    
                except Exception as e:
                    print(f"G¥î Error update leaderboard {guild.name}: {e}")
                    
        except Exception as e:
            print(f"G¥î Error auto-update leaderboard: {e}")
    ...
    async def generate_leaderboard_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        # Buat embed modern
        embed = discord.Embed(
            title="Hourly Leaderboard GÇö Top Sultan 1 Jam Terakhir",
            description="",
        ...
        if not daily_stats:
            ...
            leaderboard_lines = []
            ranking_emoji = {1: "=ƒææ", 2: "G¡É", 3: "=ƒöÑ"}
            ...
            for idx, stat in enumerate(daily_stats, 1):
                ...
                if idx in ranking_emoji:
                    ...
                    rank = "=ƒÆÄ"  # Diamond untuk rank 4-10
                
                leaderboard_lines.append(
                    f"{rank} **{name}**\n"
                    f"{stat['deals_count']} deals GÇó **{format_idr(stat['hourly_spend'])}**"
                )
        ...
        # Footer
        embed.set_footer(
            text="=ƒöä Auto-update setiap 1 jam GÇó Rolling window 1 jam",
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        if guild.id not in ALLOWED_GUILDS:
            print(f"GÜán+Å Bot di-invite ke server tidak diizinkan: {guild.name} (ID: {guild.id})")
            print(f"G£à Bot otomatis keluar dari {guild.name}")
            await guild.leave()
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        try:
            ...
            if guest_role:
                ...
                print(f"G£à Auto-role 'Guest' diberikan ke {member.name}")
            else:
                print(f"GÜán+Å Role 'Guest' tidak ditemukan di server {member.guild.name}")
            ...
            if welcome_channel:
                ...
                welcome_embed = discord.Embed(
                    description=f"## =ƒîƒ Welcome to **{member.guild.name}**! =ƒîƒ\n\n"
                                f"Halo {member.mention}! Kami senang kamu bergabung! G£¿",
                ...
                welcome_embed.add_field(
                    name="=ƒÄ» Mulai Petualangan Kamu",
                    value=f"=ƒôî Buka {channel_mention}\n=ƒôï Buat ticket pembelian\n=ƒÄü Nikmati layanan terbaik!",
                    inline=False
                )
                ...
                welcome_embed.set_footer(
                    text=f"Welcome GÇó {member.name}",
                    icon_url=member.display_avatar.url
                )
                
                await welcome_channel.send(embed=welcome_embed)
        except Exception as e:
            print(f"G¥î Error di on_member_join: {e}")
        ...
        try:
            ...
            if goodbye_channel:
                ...
                goodbye_embed = discord.Embed(
                    description=f"## =ƒÆ½ Farewell, **{member.name}** =ƒÆ½\n\n"
                ...
                goodbye_embed.add_field(
                    name="=ƒæï Sampai Jumpa",
                    value=f"```\n=ƒÆ¡ {member.name} telah pergi\n=ƒÆ¥ Terima kasih atas kebersamaannya\n=ƒîê Semoga kita bertemu lagi!\n```",
                    inline=False
                )
                
                goodbye_embed.add_field(
                    name="=ƒôè Member Tersisa",
                ...
                goodbye_embed.set_footer(
                    text=f"Goodbye GÇó We'll miss you",
                    icon_url=member.display_avatar.url
                )
                
                await goodbye_channel.send(embed=goodbye_embed)
                print(f"=ƒæï Goodbye message sent for {member.name}")
        except Exception as e:
            print(f"G¥î Error di on_member_remove: {e}")
    ...
    async def on_guild_channel_delete(self, channel: discord.TextChannel):
        ...
        try:
            ...
            if ticket and ticket['status'] == 'open':
                ...
                print(f"G£à Auto-closed ticket #{ticket['ticket_number']:04d} (channel deleted: {channel.name})")
        except Exception as e:
            print(f"G¥î Error in on_guild_channel_delete: {e}")
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            msg = (
                f"G£à Statistik untuk **{message.author.display_name}** diperbarui: "
            ...
            if new_achievements:
                achievement_names = {
                    'deals_10': '=ƒÄ» 10 Transaksi',
                    'deals_50': '=ƒöÑ 50 Transaksi',
                    'deals_100': 'G¡É 100 Transaksi',
                    'deals_500': '=ƒÆÄ 500 Transaksi',
                    'value_1m': '=ƒÆ¦ Rp1 Juta',
                    'value_5m': '=ƒÆ+ Rp5 Juta',
                    'value_10m': '=ƒÅå Rp10 Juta',
                    'value_50m': '=ƒææ Rp50 Juta',
                }
                achievement_text = ", ".join([achievement_names.get(a, a) for a in new_achievements])
                msg += f"\n=ƒÄë **Achievement Baru Unlocked!** {achievement_text}"
    ...
    async def process_proof_submission(self, message: discord.Message, ticket: dict, proof_url: str, proof_type: str = 'buyer'):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"=ƒöì PROCESSING {proof_type.upper()} PROOF: Ticket #{ticket['ticket_number']:04d}")
            print(f"{'='*60}")
            
            # === SELLER PROOF (Simpler validation) ===
            if proof_type == 'seller':
                print("=ƒôª Processing seller delivery proof...")
                ...
                if manipulation_result['manipulation_detected'] and manipulation_result['confidence'] > 60:
                    await message.channel.send(
                        ...
                        "=ƒÜ¿ **GAMBAR MENCURIGAKAN TERDETEKSI!**\n\n"
                        f"G¥î Gambar terdeteksi manipulasi (confidence: {manipulation_result['confidence']:.1f}%)\n\n"
                        "GÜán+Å Upload screenshot/bukti ASLI tanpa edit!"
                    )
                    await message.add_reaction("G¥î")
                    return
                ...
                seller_embed = discord.Embed(
                    title="=ƒôª Bukti Pengiriman Diterima",
                ...
                seller_embed.add_field(
                    name="=ƒöù Bukti Seller",
                    value=f"[=ƒô+ Klik untuk melihat]({proof_url})",
                    inline=False
                )
                
                seller_embed.add_field(
                    name="GÜÖn+Å Next Step",
                    value="Admin akan verifikasi dan release dana ke seller.\nGunakan `/approve-mm` untuk approve transaksi.",
                    inline=False
                )
                
                seller_embed.set_footer(
                    text="GÅ¦ Waiting admin approval | Middleman Transaction",
                ...
                await message.add_reaction("G£à")
                print(f"G£à Seller proof saved successfully")
                return
            
            # === BUYER PROOF (Full 4-Layer Fraud Detection) ===
            print("=ƒÆ¦ Processing buyer payment proof with 4-Layer Fraud Detection...")
            
            # === LAYER 1: TRANSFER SIGNATURE CHECK (Account + Date + Amount) ===
            print("=ƒô¥ Layer 1: Extracting transfer signature...")
            transfer_signature = await extract_transfer_signature(proof_url)
            
            if transfer_signature:
                print(f"G£à Transfer signature: {transfer_signature}")
                ...
                if exact_duplicate:
                    print(f"=ƒÜ¿ LAYER 1 FRAUD DETECTED: Signature match with Ticket #{exact_duplicate['ticket_number']:04d}")
                    ...
                    await message.channel.send(
                        f"{message.author.mention}\n"
                        "=ƒÜ¿ **TRANSFER YANG SAMA TERDETEKSI!**\n\n"
                        f"G¥î Transfer dengan **rekening, tanggal, dan nominal yang sama** sudah digunakan di:\n"
                        f"GÇó **Ticket:** `#{exact_duplicate['ticket_number']:04d}`\n"
                        f"GÇó **User:** {duplicate_user.mention if duplicate_user else 'Unknown'}\n"
                        f"GÇó **Username:** `{exact_duplicate['game_username']}`\n"
                        f"GÇó **Tanggal:** {exact_duplicate['created_at']}\n"
                        f"GÇó **Status:** `{exact_duplicate['status']}`\n\n"
                        "GÜán+Å **Gunakan screenshot ASLI dari transfer Anda!**\n"
                        "Setiap transaksi harus menggunakan bukti transfer yang berbeda.\n\n"
                        f"*(Transfer Signature: {transfer_signature[:30]}...)*"
                    )
                    
                    await message.channel.send(
                        f"=ƒÜ¿ **FRAUD ALERT:** {message.author.mention} mencoba submit transfer yang sama dengan Ticket #{exact_duplicate['ticket_number']:04d}"
                    )
                    return
                else:
                    print("G£à No signature duplicates found")
            else:
                print("GÜán+Å Could not extract transfer signature (OCR unavailable or failed)")
            
            # === LAYER 1.5: SMART FRAUD DETECTION (Whitelist + Manipulation) ===
            print("\n=ƒöì Layer 1.5: Checking screenshot legitimacy...")
            
            # STEP 1: Check if screenshot is from known legitimate source
            legitimate_check = await detect_legitimate_transfer_screenshot(proof_url)
            
            if legitimate_check['is_legitimate']:
                print(f"G£à WHITELISTED: {legitimate_check['source']} (confidence: {legitimate_check['confidence']:.1f}%)")
                print("   GÅ¡n+Å  Skipping manipulation detection for legitimate banking app screenshot")
                # BYPASS manipulation detection - proceed directly to next layer
            else:
                # STEP 2: Screenshot source unclear, run manipulation detection
                print(f"GÜán+Å Unknown source (confidence: {legitimate_check['confidence']:.1f}%) - running manipulation check...")
                manipulation_result = await detect_image_manipulation(proof_url)
                
                if manipulation_result['manipulation_detected']:
                    print(f"=ƒÜ¿ LAYER 1.5 FRAUD: Image manipulation detected!")
                    ...
                    await message.channel.send(
                        f"{message.author.mention}\n"
                        "=ƒÜ¿ **GAMBAR MENCURIGAKAN TERDETEKSI!**\n\n"
                        f"G¥î Sistem mendeteksi **manipulasi gambar** dengan confidence **{manipulation_result['confidence']:.1f}%**:\n\n"
                        f"{warning_list}\n\n"
                        "GÜán+Å **PERINGATAN:**\n"
                        "GÇó Screenshot transfer **TIDAK BOLEH** diedit/dicoret\n"
                        "GÇó Gunakan screenshot **ASLI** dari aplikasi banking/e-wallet\n"
                        "GÇó **Admin akan REJECT** bukti transfer yang diedit!\n\n"
                        "=ƒÆí **Gunakan screenshot dari:**\n"
                        "GÇó BCA Mobile, Mandiri Online, BRI Mobile\n"
                        "GÇó GoPay, OVO, Dana, ShopeePay\n"
                        "GÇó Bank lainnya (screenshot asli tanpa edit)\n\n"
                        "Silakan upload ulang bukti transfer yang ASLI."
                    )
                    
                    await message.channel.send(
                        f"=ƒÜ¿ **MANIPULATION ALERT:** {message.author.mention} upload bukti transfer dengan manipulasi (confidence: {manipulation_result['confidence']:.1f}%)"
                    )
                    
                    # CRITICAL: HARD BLOCK - Stop processing immediately!
                    # DO NOT save to database, DO NOT send confirmation embed
                    await message.add_reaction("G¥î")
                    print("=ƒÜ½ Processing stopped due to image manipulation detection")
                    return  # STOP HERE - No further processing!
            
            # === LAYER 2: PERCEPTUAL HASH CHECK (Image Similarity) ===
            print("\n=ƒÄ¿ Layer 2: Generating perceptual hash...")
            print(f"   Image URL: {proof_url[:80]}...")
            proof_hash = await get_image_hash(proof_url)
            
            if proof_hash:
                print(f"   G£à Hash generated successfully!")
                ...
                print(f"=ƒÆ+ Saving hashes to database...")
                ...
                if duplicate:
                    ...
                    if duplicate.get('is_same_ticket'):
                        print(f"=ƒÜ¿ LAYER 2 FRAUD: User trying to upload SAME image to SAME ticket!")
                        await message.channel.send(
                            f"{message.author.mention}\n"
                            "=ƒÜ¿ **GAMBAR SAMA TERDETEKSI!**\n\n"
                            f"G¥î Anda **sudah upload gambar ini** ke ticket ini sebelumnya!\n"
                            f"GÇó **Similarity:** {duplicate['similarity']}\n\n"
                            "GÜán+Å **Tidak boleh upload gambar yang sama 2x!**\n"
                    ...
                    print(f"=ƒÜ¿ LAYER 2 FRAUD DETECTED: Image similarity with Ticket #{duplicate['ticket_number']:04d}")
                    # Gambar similar terdeteksi
                    duplicate_user = await message.guild.fetch_member(int(duplicate['user_id']))
                    
                    await message.channel.send(
                        f"{message.author.mention}\n"
                        "=ƒÜ¿ **GAMBAR SERUPA TERDETEKSI!**\n\n"
                        f"G¥î Screenshot ini **{duplicate['similarity']} mirip** dengan gambar di:\n"
                        f"GÇó **Ticket:** `#{duplicate['ticket_number']:04d}`\n"
                        f"GÇó **User:** {duplicate_user.mention if duplicate_user else 'Unknown'}\n"
                        f"GÇó **Username:** `{duplicate['game_username']}`\n"
                        f"GÇó **Tanggal:** {duplicate['created_at']}\n"
                        f"GÇó **Status:** `{duplicate['status']}`\n\n"
                        "GÜán+Å **Gunakan screenshot ASLI dari transfer Anda!**\n"
                    ...
                    await message.channel.send(
                        f"GÜán+Å **FRAUD ALERT:** {message.author.mention} mencoba submit gambar mirip ({duplicate['similarity']}) dengan Ticket #{duplicate['ticket_number']:04d}"
                    )
                    return
                else:
                    print("G£à No similar images found")
            
            # Ambil data items
            items = db.get_ticket_items(ticket['id'])
            if not items:
                await message.channel.send(
                    f"{message.author.mention} G¥î Belum ada item di ticket ini. Gunakan `/add` terlebih dahulu."
                )
                return
            ...
            if detected_amount:
                ...
                if diff > tolerance:
                    ...
                    amount_warning = (
                        f"GÜán+Å **PERINGATAN NOMINAL TIDAK SESUAI!**\n"
                        f"GÇó **Terdeteksi di Screenshot:** {format_idr(detected_amount)}\n"
                        f"GÇó **Yang Seharusnya:** {format_idr(grand_total)}\n"
                        f"GÇó **Selisih:** {format_idr(diff)}\n\n"
                        f"=ƒÜ¿ Admin harap **CEK ULANG** bukti transfer ini!"
                    )
            
            # Buat embed bukti transfer
            submit_embed = discord.Embed(
                title="=ƒô¿ Bukti Transfer Diterima",
            ...
            submit_embed.add_field(name="=ƒÄ½ Ticket ID", value=f"`#{ticket['ticket_number']:04d}`", inline=True)
            submit_embed.add_field(name="=ƒæñ Username", value=f"`{ticket['game_username']}`", inline=True)
            submit_embed.add_field(name="=ƒÆ¦ Total", value=f"**{format_idr(grand_total)}**", inline=True)
            
            items_list = []
            for i in items:
                items_list.append(f"`{i['item_name']}` GÇö {format_idr(i['amount'])}")
            
            submit_embed.add_field(
                name="=ƒôª Detail Order",
                value="\n".join(items_list),
                inline=False
            )
            ...
            if detected_amount:
                ocr_status = "G£à Sesuai" if not amount_warning else f"G¥î TIDAK SESUAI (Rp{detected_amount:,})"
                submit_embed.add_field(
                    name="=ƒñû Auto-Detect Nominal",
                    value=ocr_status,
                    inline=False
                )
            ...
            if transfer_signature:
                submit_embed.add_field(
                    name="=ƒöÉ Transfer Signature",
                    value=f"`{transfer_signature[:40]}...`",
                    inline=False
                )
            
            submit_embed.add_field(
                name="=ƒöù Bukti Transfer",
                value=f"[=ƒô+ Klik untuk melihat bukti]({proof_url})",
                inline=False
            )
            
            submit_embed.add_field(
                name="GÜÖn+Å Admin Panel",
                value="G£à `/approve-ticket` GÇö Setujui transaksi\nG¥î `/reject-ticket` GÇö Tolak transaksi",
                inline=False
            )
            
            submit_embed.set_footer(
                text="GÅ¦ Menunggu verifikasi admin | 4-Layer Fraud Detection", 
                icon_url=message.guild.icon.url if message.guild.icon else None
            )
            ...
            await message.channel.send(
                f"=ƒöö {mention_text}\n\n"
                f"G£à **BUKTI TRANSFER BERHASIL TERDETEKSI!**\n"
                f"=ƒôî Ticket #{ticket['ticket_number']:04d} - {message.author.mention}\n"
                f"=ƒÆ¦ Total: **{format_idr(grand_total)}**\n\n"
                f"GÜí Fraud Detection: **PASSED** (All Layers)\n"
                f"=ƒñû Screenshot: **LEGITIMATE** (Confidence: {legitimate_check['confidence']:.1f}%)\n\n"
                f"Admin harap verifikasi menggunakan:\n"
                f"GÇó G£à `{approve_cmd}` GÇö Approve transaksi\n"
                f"GÇó G¥î `{reject_cmd}` GÇö Reject transaksi",
                embed=submit_embed
            )
            ...
            if amount_warning:
                await message.channel.send(
                    f"GÜán+Å {mention_text}\n\n{amount_warning}"
                )
                await message.add_reaction("GÜán+Å")
            else:
                await message.add_reaction("G£à")  # React checkmark ke gambar yang diupload
            
        except Exception as e:
            print(f"G¥î Error processing auto-proof: {e}")
            import traceback
            traceback.print_exc()
            await message.channel.send(
                f"{message.author.mention} G¥î Gagal memproses bukti transfer otomatis. Coba upload ulang screenshot."
            )
...
async def reset_stats(interaction: discord.Interaction, user: Optional[discord.Member] = None, reset_all: Optional[bool] = False):
    ...
    if reset_all:
        ...
        if not is_owner(interaction):
            await interaction.followup.send("G¥î Hanya Owner yang dapat mereset semua statistik!", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send(f"G¥î Gagal membuat backup: {e}", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send(f"G¥î Gagal mereset data: {e}", ephemeral=True)
            return
        
        await interaction.followup.send(
            f"G£à **Semua statistik telah di-reset!**\n"
            f"=ƒôª Backup disimpan: `{os.path.basename(backup_file)}`",
            ephemeral=False
        )
        return
    ...
    if target.id != interaction.user.id and not is_owner(interaction):
        await interaction.followup.send("G¥î Anda hanya dapat mereset statistik diri sendiri.", ephemeral=True)
        return
    ...
    await interaction.followup.send(f"G£à Statistik untuk **{target.display_name}** telah di-reset.", ephemeral=False)
...
async def reset_tickets(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        await interaction.followup.send(
            f"G£à Berhasil menghapus **{deleted_count}** ticket(s) untuk {user.mention}!",
            ephemeral=True
        )
    
    except Exception as e:
        await interaction.followup.send(f"G¥î Error: {e}", ephemeral=True)
...
async def reset_all_tickets(interaction: discord.Interaction):
    ...
    if total_tickets == 0:
        await interaction.response.send_message("Gä¦n+Å Tidak ada ticket yang perlu dihapus.", ephemeral=True)
        return
    ...
    class ConfirmResetAllView(discord.ui.View):
        ...
        @discord.ui.button(label="G£à Ya, Hapus Semua", style=discord.ButtonStyle.danger)
        async def confirm(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
            await interaction_btn.response.defer()
            self.value = True
            self.stop()
        
        @discord.ui.button(label="G¥î Batal", style=discord.ButtonStyle.secondary)
    ...
    if view.value is None:
        await interaction.followup.send("GÅ¦n+Å Waktu konfirmasi habis. Reset dibatalkan.", ephemeral=True)
        return
    
    if not view.value:
        await interaction.followup.send("G¥î Reset tickets dibatalkan.", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.followup.send(
            f"G£à **Berhasil menghapus {deleted_count} ticket(s) dari semua user!**\n"
...
async def rollback_backup(interaction: discord.Interaction, filename: Optional[str] = None):
    ...
    if not os.path.exists(backup_dir):
        await interaction.response.send_message("G¥î Tidak ada folder backup.", ephemeral=True)
        return
    ...
    if filename:
        ...
        if not os.path.exists(backup_file):
            await interaction.response.send_message(f"G¥î File backup `{filename}` tidak ditemukan.", ephemeral=True)
            return
    else:
        ...
        if not files:
            await interaction.response.send_message("G¥î Tidak ditemukan file backup.", ephemeral=True)
            return
    ...
    try:
        ...
        await interaction.response.send_message(f"G¥î Gagal melakukan restore: {e}", ephemeral=True)
        return

    resp = f"G£à Restore berhasil dari `{os.path.basename(backup_file)}`."
    if prebackup_file:
...
async def list_backups(interaction: discord.Interaction, limit: Optional[int] = 10):
    ...
    class BackupListView(discord.ui.View):
        ...
        def get_embed(self):
            ...
            embed = discord.Embed(
                title=f"=ƒôª Daftar Backup (Page {self.page + 1}/{self.max_pages})",
                color=discord.Color.blue()
            )
            ...
            for idx, path in enumerate(page_files, start=start + 1):
                ...
                embed.add_field(
                    name=f"{idx}. {fname}",
                    value=f"=ƒôà {mtime} | =ƒÆ+ {size} bytes",
                    inline=False
                )
        ...
        @discord.ui.button(label="GùÇ Previous", style=discord.ButtonStyle.secondary)
        ...
        @discord.ui.button(label="Next Gû¦", style=discord.ButtonStyle.secondary)
...
async def stats_command(interaction: discord.Interaction, user: Optional[discord.Member]):
    ...
    embed = discord.Embed(
        title=f"=ƒæñ {target_user.display_name}",
    ...
    embed.add_field(
        name="=ƒôè Total Transaksi", 
        value=f"**{deals}** deals", 
        inline=False
    )
    embed.add_field(
        name="=ƒÆ¦ Total Belanja", 
        value=f"**{format_idr(idr_value)}**\n(Gëê ${usd_value:,.2f} USD)", 
        inline=False
    )

    embed.set_footer(
        text="=ƒÆí Tip: Jangan lupa vouch setelah transaksi!", 
...
@client.tree.command(
    name="allstats",
    description="[ADMIN] =ƒôè All-Time Leaderboard - Statistik sepanjang waktu"
)
@app_commands.default_permissions(administrator=True)
@admin_or_owner()
async def allstats_command(interaction: discord.Interaction):
    ...
    try:
        ...
        if not all_stats:
            await interaction.followup.send(
                "G¥î **Belum ada data transaksi di server ini.**",
                ephemeral=True
            )
            return
        ...
        for idx, stat in enumerate(all_stats, 1):
            ...
            leaderboard_lines.append(
                f"{rank} **{name}**\n"
                f"{stat['deals_count']} deals GÇó **{format_idr(stat['total_spend'])}**"
            )
        
        # Buat embed modern
        embed = discord.Embed(
            title="All-Time Leaderboard GÇö Top Sultan",
            description="\n\n".join(leaderboard_lines),
            color=0xFFD700,  # Gold
            timestamp=dt.now()
        )
        ...
        # Footer
        embed.set_footer(
            text=f"Total {len(all_stats)} Sultan GÇó All-Time Stats",
            icon_url=interaction.guild.icon.url if interaction.guild.icon else None
        )
        ...
        if lb_weekly_channel:
            try:
                ...
                await interaction.followup.send(
                    f"G£à Leaderboard berhasil di-post ke {lb_weekly_channel.mention}",
                    ephemeral=True
                )
                print(f"G£à All-time leaderboard posted to #lb-rich-weekly")
            except Exception as e:
                await interaction.followup.send(f"G¥î Error: {e}", ephemeral=True)
                print(f"GÜán+Å Failed to post to #lb-rich-weekly: {e}")
        else:
            await interaction.followup.send(
                "G¥î Channel #lb-rich-weekly tidak ditemukan!",
                ephemeral=True
            )
            print(f"GÜán+Å Channel #lb-rich-weekly not found")
    
    except Exception as e:
        print(f"G¥î Error in /allstats: {e}")
        import traceback
        traceback.print_exc()
        await interaction.followup.send(
            f"G¥î **Error:**\n```{str(e)[:200]}```",
...
async def add_item(interaction: discord.Interaction, item: str):
    ...
    if not ticket:
        await interaction.followup.send("G¥î Command ini hanya bisa digunakan di ticket channel.", ephemeral=True)
        return
    
    if ticket['status'] != 'open':
        await interaction.followup.send("G¥î Ticket ini sudah ditutup.", ephemeral=True)
        return
    
    if int(ticket['user_id']) != interaction.user.id:
        if not interaction.user.guild_permissions.administrator:
            await interaction.followup.send("G¥î Hanya owner ticket yang bisa menambahkan item.", ephemeral=True)
            return
    ...
    if not item_data:
        await interaction.followup.send("G¥î Item tidak ditemukan.", ephemeral=True)
        return
    ...
    confirm_embed = discord.Embed(
        title="G£¿ Item Berhasil Ditambahkan",
        description="Item telah ditambahkan ke dalam order Anda.",
        color=0x9B59B6,  # Elegant purple
        timestamp=datetime.now()
    )
    
    confirm_embed.add_field(name="=ƒ¢ìn+Å Item", value=f"`{item_name}`", inline=True)
    confirm_embed.add_field(name="=ƒÆÄ Robux", value=f"`{robux} R$`", inline=True)
    confirm_embed.add_field(name="=ƒÆ¦ Harga", value=format_idr(unit_price), inline=True)
    ...
    confirm_embed.add_field(
        name="=ƒôª Ringkasan Order",
        value="\n".join(items_list),
        inline=False
    )
    
    confirm_embed.add_field(
        name="=ƒÆ¦ Total Pembayaran",
        value=f"**{format_idr(grand_total)}**",
        inline=False
    )
    
    confirm_embed.set_footer(text="=ƒÆí Tip: Upload screenshot bukti transfer langsung ke channel ini", icon_url=interaction.guild.icon.url if interaction.guild.icon else None)
...
async def add_item_autocomplete(interaction: discord.Interaction, current: str):
    ...
    return [
        app_commands.Choice(
            name=f"{item['name']} ({item['robux']} R$ GÇó {format_idr(item['price_idr'])})",
            value=item['code']
        )
...
async def approve_ticket(interaction: discord.Interaction):
    try:
        ...
        print(f"\n{'='*80}")
        print(f"[{timestamp}] =ƒöì [APPROVE-TICKET] COMMAND STARTED")
        print(f"{'='*80}")
        ...
        if ticket:
            print(f"   G£à Ticket #: {ticket.get('ticket_number')}")
            print(f"   G£à Status: {ticket.get('status')}")
        else:
            print(f"   G¥î NO TICKET FOUND FOR THIS CHANNEL!")
        
        # Defer FIRST to avoid timeout
        await interaction.response.defer(ephemeral=True)
        print(f"   G£à Interaction deferred")
        ...
        if not items:
            await interaction.followup.send("G¥î Tidak ada item di ticket ini.", ephemeral=True)
            return
        ...
        class ValidationChecklist(discord.ui.View):
                ...
                for item in self.children:
                    if isinstance(item, discord.ui.Button):
                        if item.custom_id == "check_account":
                            item.label = "Gÿæ Nomor Rekening Cocok" if self.checks['account'] else "GÿÉ Nomor Rekening Cocok"
                            ...
                            item.label = "Gÿæ Timestamp Transfer Wajar" if self.checks['timestamp'] else "GÿÉ Timestamp Transfer Wajar"
                            ...
                            item.label = "Gÿæ Nominal Sesuai" if self.checks['amount'] else "GÿÉ Nominal Sesuai"
                            ...
                            item.label = "Gÿæ Screenshot Asli (Tidak Edit)" if self.checks['screenshot'] else "GÿÉ Screenshot Asli (Tidak Edit)"
                            item.style = discord.ButtonStyle.success if self.checks['screenshot'] else discord.ButtonStyle.secondary
            
            @discord.ui.button(label="GÿÉ Nomor Rekening Cocok", style=discord.ButtonStyle.secondary, custom_id="check_account")
            async def check_account(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
                ...
                button.label = "Gÿæ Nomor Rekening Cocok" if self.checks['account'] else "GÿÉ Nomor Rekening Cocok"
            ...
            @discord.ui.button(label="GÿÉ Timestamp Transfer Wajar", style=discord.ButtonStyle.secondary, custom_id="check_time")
            async def check_timestamp(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
                ...
                button.label = "Gÿæ Timestamp Transfer Wajar" if self.checks['timestamp'] else "GÿÉ Timestamp Transfer Wajar"
                button.style = discord.ButtonStyle.success if self.checks['timestamp'] else discord.ButtonStyle.secondary
                await interaction_btn.response.edit_message(view=self)
            
            @discord.ui.button(label="GÿÉ Nominal Sesuai", style=discord.ButtonStyle.secondary, custom_id="check_amount")
            async def check_amount(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
                ...
                button.label = "Gÿæ Nominal Sesuai" if self.checks['amount'] else "GÿÉ Nominal Sesuai"
                button.style = discord.ButtonStyle.success if self.checks['amount'] else discord.ButtonStyle.secondary
                await interaction_btn.response.edit_message(view=self)
            
            @discord.ui.button(label="GÿÉ Screenshot Asli (Tidak Edit)", style=discord.ButtonStyle.secondary, custom_id="check_ss")
            async def check_screenshot(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
                ...
                button.label = "Gÿæ Screenshot Asli (Tidak Edit)" if self.checks['screenshot'] else "GÿÉ Screenshot Asli (Tidak Edit)"
                button.style = discord.ButtonStyle.success if self.checks['screenshot'] else discord.ButtonStyle.secondary
                await interaction_btn.response.edit_message(view=self)
            
            @discord.ui.button(label="GÜí CEK SEMUA", style=discord.ButtonStyle.primary, custom_id="check_all", row=1)
            ...
            @discord.ui.button(label="G£à APPROVE SEKARANG", style=discord.ButtonStyle.danger, custom_id="confirm_approve", row=1)
            async def confirm_approve(self, interaction_btn: discord.Interaction, button: discord.ui.Button):
                ...
                if not all(self.checks.values()):
                    ...
                    await interaction_btn.response.send_message(
                        f"GÜán+Å **Checklist belum lengkap!**\n\n"
                        f"Belum divalidasi: {', '.join(unchecked)}\n\n"
                        f"Pastikan semua checklist sudah G£à sebelum approve.",
                        ephemeral=True
                    )
                    return
            ...
            @discord.ui.button(label="G¥î Batal", style=discord.ButtonStyle.secondary, custom_id="cancel_approve", row=1)
        ...
        checklist_embed = discord.Embed(
            title="=ƒöÉ Validasi Bukti Transfer",
            description=(
                f"**Admin:** {interaction.user.mention}\n"
                f"**Ticket:** #{ticket['ticket_number']:04d}\n"
                f"**Total:** {format_idr(grand_total)}\n\n"
                f"GÜán+Å **WAJIB CEK SEMUA** sebelum approve:\n"
                f"Klik setiap item untuk tandai sudah dicek G£à"
            ),
            color=0xE74C3C
        )
        checklist_embed.set_footer(text="GÅ¦n+Å Timeout: 2 menit GÇó Anti-Fraud System")
        ...
        if not view.confirmed:
            await interaction.followup.send("G¥î Approval dibatalkan.", ephemeral=True)
            return
        
        # Proceed with approval
        await interaction.followup.send("GÅ¦ Memproses approval...", ephemeral=True)
        ...
        if new_achievements:
            ach_names = {
                'deals_10': '=ƒÄ» 10 Deals',
                'deals_50': '=ƒöÑ 50 Deals',
                'deals_100': 'G¡É 100 Deals',
                'deals_500': '=ƒÆÄ 500 Deals',
                'value_1m': '=ƒÆ¦ Rp1M',
                'value_5m': '=ƒÆ+ Rp5M',
                'value_10m': '=ƒÅå Rp10M',
                'value_50m': '=ƒææ Rp50M',
            ...
            success_embed.add_field(
                name="=ƒÄë Achievement",
                value=" GÇó ".join(ach_list),
                inline=False
            )
        ...
        if buyer:
            ...
            if cust_role and cust_role not in buyer.roles:
                try:
                    ...
                    print(f"G£à Auto-role 'CUST' diberikan ke {buyer.name}")
                    ...
                    if guest_role and guest_role in buyer.roles:
                        ...
                        print(f"=ƒùæn+Å Role 'Guest' dihapus dari {buyer.name} (upgrade ke CUST)")
                        
                except Exception as e:
                    print(f"GÜán+Å Gagal memberikan role CUST: {e}")
        ...
        try:
            ...
            print(f"G¥î Error deleting channel: {e}")
    
    except Exception as e:
        print(f"G¥î Error in approve_ticket: {e}")
        try:
            await interaction.followup.send(f"G¥î Error: {e}", ephemeral=True)
        except:
            pass
...
async def trigger_leaderboard_update(guild: discord.Guild):
    ...
    try:
        ...
        try:
            ...
            print(f"G£à Leaderboard updated setelah transaksi di {guild.name}")
            
        except Exception as e:
            print(f"G¥î Error update leaderboard: {e}")
            
    except Exception as e:
        print(f"G¥î Error trigger leaderboard update: {e}")
...
async def post_stats_to_cmd_bot(interaction, ticket, grand_total, new_achievements):
    ...
    try:
        ...
        if not cmd_bot_channel:
            print("GÜán+Å #cmd-bot channel not found, skipping stats post")
            return
        ...
        if not buyer or not stats:
            print("GÜán+Å Buyer or stats not found")
            return
        ...
        stats_embed = discord.Embed(
            title=f"=ƒæñ {buyer.display_name}",
            description="Statistik Transaksi",
        ...
        # Tip untuk vouch
        stats_embed.add_field(
            name="=ƒÆí Tip: Jangan lupa vouch setelah transaksi!",
            value="\u200b",
            inline=False
        )
        ...
        stats_embed.set_footer(
            text=f"G£à Ticket #{ticket['ticket_number']:04d} GÇó Approved by {interaction.user.name}",
            icon_url=interaction.guild.icon.url if interaction.guild.icon else None
        )
        ...
        if existing_msg:
            ...
            try:
                ...
                if channel:
                    ...
                    print(f"G£à Updated stats in #cmd-bot for {buyer.name}")
                    return
            except discord.NotFound:
                # Message sudah dihapus, post baru
                print(f"GÜán+Å Old stats message not found, posting new one")
            except Exception as e:
                print(f"GÜán+Å Error updating stats message: {e}")
        ...
        print(f"G£à Posted new stats to #cmd-bot for {buyer.name}")
        
    except Exception as e:
        print(f"G¥î Error posting stats to #cmd-bot: {e}")
...
async def reject_ticket(interaction: discord.Interaction, reason: str = "Bukti transfer tidak valid"):
    ...
    reject_embed = discord.Embed(
        title="G¥î Transaksi Ditolak",
        description=f"Pembayaran tidak dapat diverifikasi oleh admin.",
        color=0xE74C3C,  # Elegant red
        timestamp=datetime.now()
    )
    
    reject_embed.add_field(name="=ƒô¥ Alasan Penolakan", value=reason, inline=False)
    reject_embed.add_field(name="=ƒÆí Saran", value="Silakan hubungi admin untuk klarifikasi lebih lanjut.", inline=False)
    reject_embed.set_footer(text="=ƒòÆ Channel akan ditutup dalam 30 detik", icon_url=interaction.guild.icon.url if interaction.guild.icon else None)
    
    await interaction.channel.send(embed=reject_embed)
    await interaction.followup.send("G¥î Rejected! Channel akan dihapus dalam 30 detik.")
...
async def close_ticket(interaction: discord.Interaction):
    try:
        ...
        if int(ticket['user_id']) != interaction.user.id and not interaction.user.guild_permissions.administrator:
            await interaction.followup.send("G¥î Hanya owner ticket atau admin yang bisa close ticket.", ephemeral=True)
            return
        
        db.close_ticket(ticket['id'], interaction.user.id)
        
        close_embed = discord.Embed(
            title="=ƒöÆ Ticket Ditutup",
            description=f"Ticket telah ditutup oleh {interaction.user.mention}",
            color=0xE67E22,  # Elegant orange
            timestamp=datetime.now()
        )
        
        close_embed.add_field(name="=ƒÆ¼ Catatan", value="Terima kasih telah menggunakan layanan kami.", inline=False)
        close_embed.set_footer(text="=ƒòæ Channel akan dihapus dalam 10 detik", icon_url=interaction.guild.icon.url if interaction.guild.icon else None)
        
        await interaction.followup.send("G£à Ticket ditutup. Channel akan dihapus dalam 10 detik.")
        ...
        print(f"G¥î Error in close_ticket: {e}")
        try:
            await interaction.followup.send(f"G¥î Error: {e}", ephemeral=True)
        except:
            pass
...
async def setup_ticket_channel(interaction: discord.Interaction):
    ...
    if existing_channel:
        await interaction.followup.send(
            f"G£à Channel {existing_channel.mention} sudah ada.\n"
    ...
    try:
        ...
        channel = await interaction.guild.create_text_channel(
            ...
            topic="=ƒô¥ Ketik username game Anda di sini untuk membuka ticket order"
        )
        ...
        instruction_embed.add_field(
            name="GÜí Penting",
            value=(
                "GÇó 1 user hanya bisa punya 1 ticket aktif\n"
                "GÇó Upload bukti transfer ASLI (tidak boleh edit)\n"
                "GÇó Button akan tetap ada meski bot restart\n"
                "GÇó Pilih item langsung dari dropdown menu"
            ),
            inline=False
        )
        
        instruction_embed.set_footer(text="=ƒÄ« Fish-It Roblox Gift Service GÇó Trusted Seller")
        ...
        await interaction.followup.send(
            f"G£à Channel {channel.mention} berhasil dibuat{category_info}!\n\n"
...
async def setup_mm_channel(interaction: discord.Interaction):
    ...
    try:
        ...
        channel = await interaction.guild.create_text_channel(
            ...
            topic="=ƒñ¥ Klik button untuk membuka middleman ticket"
        )
        
        # Kirim instruksi di channel
        instruction_embed = discord.Embed(
            title="=ƒñ¥ Middleman Service - Panduan",
        ...
        instruction_embed.add_field(
            ...
            value=(
                ...
                "**3.** Buyer transfer GåÆ Seller kirim item GåÆ Admin release dana"
            ),
            inline=False
        )
        
        instruction_embed.add_field(
            name="=ƒÆ¦ Fee Middleman",
            value=(
                "```\n"
                "< Rp50K      : GRATIS\n"
                "Rp50K-500K   : Rp2.000\n"
        ...
        instruction_embed.add_field(
            name="G£à Keuntungan",
            value=(
                "=ƒ¢ín+Å **Aman** - Dana ditahan sampai item diterima\n"
                "GÜí **Anti-Fraud** - Auto-reject bukti palsu\n"
                "=ƒÆ+ **Murah** - Gratis untuk transaksi <50K"
            ),
            inline=False
        )
        
        instruction_embed.set_footer(text="Klik tombol untuk mulai GÇó Trusted Service")
        ...
        # Konfirmasi ke admin
        await interaction.followup.send(
            f"G£à Channel {channel.mention} berhasil dibuat di kategori **{mm_category.name}**!\n\n"
    ...
    try:
        if amount is None:
            ...
            msg = await interaction.followup.send(
                f"G£à Berhasil menghapus **{deleted_total}** pesan dari {interaction.channel.mention}!",
                ephemeral=True
            )
        else:
            ...
            msg = await interaction.followup.send(
                f"G£à Berhasil menghapus **{len(deleted)}** pesan dari {interaction.channel.mention}!",
                ephemeral=True
            )
        ...
        msg = await interaction.followup.send(
            "G¥î Bot tidak punya permission untuk menghapus pesan.\n"
...
async def set_admin(interaction: discord.Interaction, role: discord.Role):
    ...
    try:
        ...
        if str(role.id) in admin_roles:
            await interaction.followup.send(
                f"G¥î Role {role.mention} sudah menjadi Admin!",
                ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"G£à Role {role.mention} berhasil ditambahkan sebagai **Admin Bot**!\n\n"
            f"Admin sekarang bisa:\n"
            f"GÇó Approve/Reject ticket\n"
            f"GÇó Lihat statistik semua user\n"
            f"GÇó Hapus pesan (clear)\n"
            f"GÇó Backup/restore data\n"
            f"GÇó Monitor transaksi",
            ephemeral=True
        )
...
async def remove_admin(interaction: discord.Interaction, role: discord.Role):
    ...
    try:
        ...
        if str(role.id) not in admin_roles:
            await interaction.followup.send(
                f"G¥î Role {role.mention} bukan Admin!",
                ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"G£à Role {role.mention} berhasil dihapus dari Admin Bot!",
            ephemeral=True
        )
    ...
    try:
        ...
        if admin_roles:
            ...
            for role_id in admin_roles:
                role = interaction.guild.get_role(int(role_id))
                if role:
                    role_mentions.append(f"GÇó {role.mention}")
            
            embed.add_field(
                name="=ƒ¢ín+Å Admin Roles",
        ...
        # Admin Permissions
        embed.add_field(
            name="G£à Admin Dapat:",
            value=(
                "GÇó `/approve-ticket` - Approve transaksi\n"
                "GÇó `/reject-ticket` - Reject transaksi\n"
                "GÇó `/ticket-stats` - Lihat statistik ticket\n"
                "GÇó `/user-info` - Info detail user\n"
                "GÇó `/audit-log` - Lihat log aktivitas\n"
                "GÇó `/list-admins` - Lihat daftar admin\n"
                "GÇó `/addrole` - Kasih role ke user\n"
                "GÇó `/removerole` - Hapus role dari user"
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="=ƒææ Owner Only:",
            value=(
                "GÇó `/set-admin` - Tambah admin role\n"
                "GÇó `/remove-admin` - Hapus admin role\n"
                "GÇó `/setup-ticket` - Setup channel ticket\n"
                "GÇó `/reset-tickets` - Reset semua ticket\n"
                "GÇó `/broadcast` - Broadcast message\n"
                "GÇó `/clear` - Hapus banyak pesan\n"
                "GÇó **Full Access** ke semua command"
            ),
            inline=False
        )
        
        embed.set_footer(text="GÜán+Å Admin role dibatasi untuk keamanan GÇó Hanya Owner full access")
...
async def permissions(interaction: discord.Interaction):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"=ƒöÉ Permission Level: {level}",
            color=color,
            timestamp=datetime.now()
        )
        ...
        if is_owner_user:
            embed.add_field(
                name="G£à Owner Commands (Full Access):",
                value=(
                    "**Setup & Config:**\n"
                    "GÇó `/set-admin` - Manage admin roles\n"
                    "GÇó `/remove-admin` - Remove admin roles\n"
                    "GÇó `/setup-ticket` - Setup ticket system\n"
                    "GÇó `/reset-tickets` - Reset all tickets\n\n"
                    "**Management:**\n"
                    "GÇó `/broadcast` - Send announcements\n"
                    "GÇó `/clear` - Bulk delete messages\n\n"
            ...
            embed.add_field(
                name="G£à Admin Commands:",
                value=(
                    "**Ticket Management:**\n"
                    "GÇó `/approve-ticket` - Approve transactions\n"
                    "GÇó `/reject-ticket` - Reject transactions\n\n"
                    "**User Management:**\n"
                    "GÇó `/addrole` - Give role to user\n"
                    "GÇó `/removerole` - Remove role from user\n\n"
                    "**Monitoring:**\n"
                    "GÇó `/ticket-stats` - View ticket statistics\n"
                    "GÇó `/user-info` - View user details\n"
                    "GÇó `/audit-log` - View activity logs\n"
                    "GÇó `/list-admins` - View admin list\n\n"
            ...
            embed.add_field(
                name="G¥î Tidak Bisa:",
                value=(
                    "GÇó Setup/Reset system\n"
                    "GÇó Manage admin roles\n"
                    "GÇó Broadcast messages\n"
                    "GÇó Bulk delete messages"
                ),
                inline=False
            )
        else:
            embed.add_field(
                name="G£à User Commands:",
                value=(
                    "GÇó `/stats` - View your statistics\n"
                    "GÇó `/leaderboard` - View top spenders\n"
                    "GÇó `/close` - Close your ticket\n"
                    "GÇó `/permissions` - Check your permissions\n"
                    "GÇó **Create tickets** via #open-ticket button"
                ),
                inline=False
            )
...
async def user_info(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"=ƒæñ User Info: {user.display_name}",
            color=discord.Color.blue(),
            timestamp=datetime.now()
        )
        ...
        if stats:
            ...
            if achievements:
                ach_icons = {
                    'deals_10': '=ƒÄ»', 'deals_50': '=ƒöÑ', 'deals_100': 'G¡É', 'deals_500': '=ƒÆÄ',
                    'value_1m': '=ƒÆ¦', 'value_5m': '=ƒÆ+', 'value_10m': '=ƒÅå', 'value_50m': '=ƒææ'
                }
                ach_list = [f"{ach_icons.get(a['achievement_type'], '=ƒÅà')} {a['achievement_type']}" 
                           for a in achievements]
                
                embed.add_field(
                    name=f"=ƒÅå Achievements ({len(achievements)})",
                    value="\n".join(ach_list[:5]),
                    inline=False
                )
        else:
...
async def addrole(interaction: discord.Interaction, user: discord.Member, role: discord.Role):
    ...
    try:
        ...
        if role in user.roles:
            await interaction.followup.send(
                f"G¥î {user.mention} sudah memiliki role {role.mention}!",
                ephemeral=True
            )
            return
        ...
        if role.position >= interaction.guild.me.top_role.position:
            await interaction.followup.send(
                f"G¥î Role {role.mention} lebih tinggi dari role bot. Bot tidak bisa manage role ini!",
                ephemeral=True
            )
            return
        ...
        if role.position >= interaction.user.top_role.position and not is_owner(interaction):
            await interaction.followup.send(
                f"G¥î Role {role.mention} lebih tinggi dari role kamu!",
                ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"G£à Berhasil memberikan role {role.mention} ke {user.mention}!",
            ephemeral=True
        )
        ...
        await interaction.followup.send(
            "G¥î Bot tidak punya permission untuk manage roles!",
            ephemeral=True
        )
    except Exception as e:
        await interaction.followup.send(f"G¥î Error: {e}", ephemeral=True)
...
async def removerole(interaction: discord.Interaction, user: discord.Member, role: discord.Role):
    ...
    try:
        ...
        if role not in user.roles:
            await interaction.followup.send(
                f"G¥î {user.mention} tidak memiliki role {role.mention}!",
                ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"G£à Berhasil menghapus role {role.mention} dari {user.mention}!",
            ephemeral=True
        )
...
async def set_rate(interaction: discord.Interaction, rate: int):
    ...
    if rate < 10:
        await interaction.followup.send("G¥î Rate minimal Rp10 per Robux", ephemeral=True)
        return
    
    if rate > 1000:
        await interaction.followup.send("G¥î Rate maksimal Rp1.000 per Robux", ephemeral=True)
        return
    ...
    if setup_data:
        try:
            ...
            if channel:
                ...
                items_text = []
                for item in items:
                    items_text.append(f"**{item['name']}:** {item['robux']} R$ GÇó {format_idr(item['price_idr'])}")
                ...
                instruction_embed.add_field(
                    ...
                    value=(
                        ...
                        "GÇó Upload bukti transfer ASLI (tidak boleh edit)\n"
                        "GÇó Button akan tetap ada meski bot restart\n"
                        "GÇó Gunakan `/add` di ticket untuk order"
                    ),
                    inline=False
                )
                ...
                await interaction.followup.send(
                    f"G£à **Rate berhasil diupdate!**\n\n"
                    f"**Rate Lama:** Rp{old_rate}/Robux\n"
                    f"**Rate Baru:** Rp{rate}/Robux\n\n"
                    f"**Perubahan Harga:**\n" +
                    "\n".join([f"GÇó {item['name']}: {format_idr(item['robux'] * old_rate)} GåÆ {format_idr(item['price_idr'])}" for item in items[:5]]) +
                    f"\n\n=ƒôî Display di {channel.mention} sudah auto-update!",
                    ephemeral=True
                )
        except Exception as e:
            await interaction.followup.send(
                f"G£à Rate diupdate ke Rp{rate}/Robux\n\n"
                f"GÜán+Å Gagal update display: {e}\n"
        ...
        await interaction.followup.send(
            ...
            f"**Rate Lama:** Rp{old_rate}/Robux\n"
            f"**Rate Baru:** Rp{rate}/Robux\n\n"
            f"=ƒÆí Gunakan `/setup-ticket` untuk setup channel dengan harga baru.",
            ephemeral=True
        )
    ...
    db.log_action(
        ...
        user_id=interaction.user.id,
        action="set_rate",
        details=f"Rate updated: Rp{old_rate} GåÆ Rp{rate} per Robux"
    )
...
async def add_item_to_catalog(interaction: discord.Interaction, name: str, robux: int):
    ...
    if robux < 1:
        await interaction.followup.send("G¥î Harga Robux minimal 1", ephemeral=True)
        return
    
    if robux > 10000:
        await interaction.followup.send("G¥î Harga Robux maksimal 10.000", ephemeral=True)
        return
    ...
    if existing:
        await interaction.followup.send(f"G¥î Item dengan nama mirip sudah ada! Gunakan nama berbeda.", ephemeral=True)
        return
    ...
    if setup_data:
        try:
            ...
            if channel:
                ...
                await interaction.followup.send(
                    f"G£à **Item berhasil ditambahkan!**\n\n"
                    f"**Kode:** `{code}`\n"
                    f"**Nama:** {name}\n"
                    f"**Harga:** {robux} R$ GÇó {format_idr(price_idr)}\n\n"
                    f"=ƒôî Display di #open-ticket sudah auto-update!\n"
                    f"=ƒÆí Total item sekarang: {len(items)}",
                    ephemeral=True
                )
        except Exception as e:
            await interaction.followup.send(
                f"G£à Item **{name}** berhasil ditambahkan ({robux} R$ GÇó {format_idr(price_idr)})\n\n"
                f"GÜán+Å Gagal update display: {e}",
                ephemeral=True
            )
    else:
    ...
    if setup_data:
        try:
            ...
            if channel:
                ...
                await interaction.followup.send(
                    f"G£à **Item berhasil dihapus!**\n\n"
                    f"**Nama:** {item_data['name']}\n"
                    f"**Harga:** {item_data['robux']} R$ GÇó {format_idr(item_data['price_idr'])}\n\n"
                    f"=ƒôî Display di #open-ticket sudah auto-update!\n"
                    f"=ƒÆí Total item sekarang: {len(items)}",
                    ephemeral=True
                )
            ...
            await interaction.followup.send(
                f"G£à Item **{item_data['name']}** berhasil dihapus\n\n"
                f"GÜán+Å Gagal update display: {e}",
                ephemeral=True
            )
    else:
...
async def remove_item_autocomplete(
    ...
    try:
        ...
        return [
            app_commands.Choice(
                name=f"{item['name']} ({item['robux']} R$ GÇó Rp{item['price_idr']:,})",
                value=item['code']
            )
            for item in items[:25]
        ]
...
async def daily_leaderboard(interaction: discord.Interaction):
    ...
    try:
        ...
        if existing_msg:
            try:
                ...
                if old_channel:
                    ...
                    await old_message.delete()
                    print(f"=ƒùæn+Å Deleted old leaderboard message")
            except:
                pass
        ...
        # Footer
        embed.set_footer(
            text="Auto-update setiap 1 jam GÇó Rolling window 1 jam",
            icon_url=interaction.guild.icon.url if interaction.guild.icon else None
        )
        ...
        await interaction.followup.send(
            f"G£à **Daily leaderboard berhasil di-setup!**\n\n"
            f"=ƒôì Channel: {lb_channel.mention}\n"
            f"=ƒöä Auto-update: Setiap 1 jam\n"
            f"=ƒôè Message ID: `{message.id}`",
            ephemeral=True
        )
        
        print(f"G£à Daily leaderboard setup: {interaction.guild.name} GåÆ {lb_channel.name}")
        
    except discord.Forbidden:
        await interaction.followup.send(
            f"G¥î Bot tidak punya permission untuk post di {lb_channel.mention}!",
            ephemeral=True
        )
    except Exception as e:
        await interaction.followup.send(
        ...
        print(f"G¥î Error in /daily-leaderboard: {e}")
        import traceback
        traceback.print_exc()
...
async def approve_mm(interaction: discord.Interaction):
    try:
        ...
        if not ticket:
            await interaction.response.send_message("G¥î Command ini hanya bisa digunakan di middleman ticket channel.", ephemeral=True)
            return
        
        if ticket.get('ticket_type') != 'middleman':
            await interaction.response.send_message("G¥î Ini bukan middleman ticket. Gunakan `/approve-ticket` untuk ticket purchase.", ephemeral=True)
            return
        
        if ticket['status'] != 'open':
            await interaction.response.send_message("G¥î Ticket ini sudah ditutup.", ephemeral=True)
            return
        
        # Verify buyer has submitted proof
        if not ticket.get('proof_url'):
            await interaction.response.send_message("G¥î Buyer belum upload bukti transfer!", ephemeral=True)
            return
        ...
        completion_embed = discord.Embed(
            title="G£à Transaksi Middleman Berhasil!",
        ...
        completion_embed.set_footer(
            text=f"Approved by {interaction.user.name} GÇó Ticket closed",
        ...
        await interaction.followup.send(
            f"G£à **Middleman transaksi berhasil diapprove!**\n\n"
            f"=ƒôî Ticket #{ticket['ticket_number']:04d} ditutup\n"
            f"=ƒÆ¦ Seller payout: **Rp{seller_payout:,}**\n"
            f"=ƒÆ¦ Fee middleman: **Rp{mm_fee:,}**\n\n"
            f"GÜán+Å **JANGAN LUPA:** Transfer Rp{seller_payout:,} ke seller `{seller_username}`!",
            ephemeral=True
        )
...
async def reject_mm(interaction: discord.Interaction, reason: str = "Bukti tidak valid"):
    try:
        ...
        if ticket.get('ticket_type') != 'middleman':
            await interaction.response.send_message("G¥î Ini bukan middleman ticket. Gunakan `/reject-ticket` untuk ticket purchase.", ephemeral=True)
            return
        ...
        reject_embed = discord.Embed(
            title="G¥î Transaksi Middleman Ditolak",
        ...
        await interaction.followup.send(
            f"G£à Middleman ticket #{ticket['ticket_number']:04d} berhasil direject!\n"
...
if __name__ == '__main__':
    ...
    async def on_ready():
        try:
            await send_log_message(client, f"G£à Bot berhasil di-restart dan online! (PID: {os.getpid()})")
        except Exception:
            pass
    ...
    try:
        ...
        print("G¥î ERROR: Token bot tidak valid. Cek kembali token di file .env.")
        try:
            import asyncio
            asyncio.run(send_log_message(client, "G¥î ERROR: Token bot tidak valid. Cek kembali token di file .env."))
        except Exception:
            pass
    except Exception as e:
        import traceback
        error_text = f"G¥î ERROR: Terjadi kesalahan saat menjalankan bot: {e}\n{traceback.format_exc()}"
        print(error_text)
        try:
            import asyncio
